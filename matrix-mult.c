...
typedef struct {
// What arguments do you need to send to each thread?
   int row;
   int col;
   int size;
} instruction_t;
...
int main(int argc, char *argv[]) {
   ...
   int num_threads = col_a * row_b; // How many threads will there be?
   ...
   int t_index = 0;
	for (int i = 0; i< col_a; ++i) {
		for (int j = 0; j< row_b; ++j) {
			...
			inst->col = j;
			inst->size = col_a;
			pthread_create(&tids[t_index], &attr, cell_multiply, inst);
			++t_index;
		}
	}
...
void *cell_multiply(void *param) {
   ...
   int result = 0;
	for (int i = 0; i< inst->size; ++i) {
		result += matrix_a[inst->row][i] * matrix_b[i][inst->col];
	}
	matrix_c[inst->row][inst->col] = result;
}
